...
def Add(count): #to define add members
    ...
    print("BMI is " + str(BMI), '\nType is ' + str(result(BMI))) #to tell the user the BMI and type of their inputs, this is the ouput
...
def ViewAll(): #to define the view all 
    print("\n=== List of members ===") #list of members will be printed out 
...
def Edit(Information): #to define the edit options
    ...
    for k in Information: #for every keys in Information dictionary
        if member_id == k: #if the member_id is equal to k or keys, it will ask the user about the new weight
            ...
            bmi = edit_weight / (height / 100) ** 2 #the calculation of the bmi
...
def DelAll(): #to define the delete all
    ...
    return("All entries successfully deted.") #after deleting, it will return this statement to tell the user that the contents of dictionary has been deleted
    


def choice(): #for the choice of users, we set an
    print("\nWelcome to EBR Fitness Club")
